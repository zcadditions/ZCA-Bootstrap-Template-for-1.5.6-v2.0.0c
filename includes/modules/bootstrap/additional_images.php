...
if ($num_images > 0) {
    ...
    if (ZCA_PHOTOSWIPE_STATUS == 'true') {         
        $height = '1024';
        $width = '1024';
            $script_link = '<a href="' . zca_photoswipe($products_image_large, '', LARGE_IMAGE_WIDTH, LARGE_IMAGE_HEIGHT) . '" itemprop="contentUrl" data-size="'.$width.'x'.$height.'" class="mx-auto d-block img-fluid"><img src="' . zca_photoswipe($base_image, $products_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '" itemprop="thumbnail" class="mx-auto d-block img-fluid" /></a><figcaption itemprop="caption description">' . addslashes($products_name) . '</figcaption></figure>';
    } else { 
        $script_link = false;
        $link_parameters = '';
        $zco_notifier->notify(

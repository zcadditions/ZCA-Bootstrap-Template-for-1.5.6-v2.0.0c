...
// shopping_cart: Provide updated processing **ONLY IF** the ZCA bootstrap is the active template.
//
...
//  $showFixedQuantityAmount = $products[$i]['quantity'] . zen_draw_hidden_field('cart_quantity[]', 1);
  ...
  $buttonUpdate = ((SHOW_SHOPPING_CART_UPDATE == 1 or SHOW_SHOPPING_CART_UPDATE == 3) ? '<button type="submit" class="btn"><i class="fas fa-sync-alt"></i></button>' : '') . zen_draw_hidden_field('products_id[]', $products[$i]['id']);

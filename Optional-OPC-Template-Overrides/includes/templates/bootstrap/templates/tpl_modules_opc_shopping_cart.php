...
<div id="opcShoppingCart" class="card p-0 mb-3">

    <?php echo '<h4 class="card-header">' . HEADING_PRODUCTS . '</h4>'; ?>

  <div class="card-body p-3">

   
     <table class="table table-bordered table-striped" id="cartContentsDisplay">
        <tr>
            <td class="edit-button" colspan="<?php echo (count($order->info['tax_groups']) > 1) ? 3 : 2; ?>">&nbsp;</td>
            <td class="edit-button text-center"><?php echo '<a href="' . zen_href_link(FILENAME_SHOPPING_CART) . '">' . zen_image_button(BUTTON_IMAGE_EDIT_SMALL, BUTTON_EDIT_SMALL_ALT) . '</a>'; ?></td>
        </tr>
        
        <tr class="cartTableHeading">
          <th scope="col" id="ccQuantityHeading" class="text-center"><?php echo TABLE_HEADING_QUANTITY; ?></th>
...
?>
          <th scope="col" id="ccTotalHeading" class="text-center"><?php echo TABLE_HEADING_TOTAL; ?></th>
        </tr>
<?php 
...
?>
        <tr class="<?php echo $order->products[$i]['rowClass']; ?>">
          <td class="cartQuantity text-center"><?php echo $order->products[$i]['qty']; ?>&nbsp;x</td>
...
<?php
    ...
    if (isset ($posStockMessage)) {
        echo '<div class="p-3"></div>' . $posStockMessage[$i];
    }
?>
          </td>
<?php 
